...
export const loadDataAsync = ((data) => (
  dataType = FlagType.EMOJI,
) => {
...
const isCountryPresent = (countries) => (
  countryCode,
) => !!countries[countryCode]

const isRegion = (region) => (country) =>
  region ? country.region === region : true

const isSubregion = (subregion) => (country) =>
  subregion ? country.subregion === subregion : true

const isIncluded = (countryCodes) => (country) =>
  countryCodes && countryCodes.length > 0
    ? countryCodes.includes(country.cca2)
    : true

const isExcluded = (excludeCountries) => (country) =>
...
export const getLetters = (countries) => {
  return uniq(
    countries
      .map((country) =>
        (country.name ).substr(0, 1).toLocaleUpperCase(),
      )
